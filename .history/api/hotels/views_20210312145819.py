...
def scheduleRoom(request):
    ...
    if request.method == 'GET':
        ...
        checked_out=False
        if ScheduledRoom.objects.filter(room_no=room_no).exists():
            return JsonResponse({'booked':False})
        else:
            scheduleRoom=ScheduledRoom.objects.create(hotel_id_id=hotel_id,username=user,room_no=room_no,checkin=checkin,checkout=checkout,checked_out=checked_out)
            scheduleRoom.save()

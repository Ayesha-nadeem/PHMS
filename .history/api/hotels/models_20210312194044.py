...
def scheduledRoom_post_save(instance, *args, **kwargs):
    update_checked_out.apply_async((instance, str(instance.username)), eta=instance.checkout)
...
def update_checked_out(instance,user):
    ...
    data = serializers.serialize('json', instance.objects.filter(username=user))
    try:
